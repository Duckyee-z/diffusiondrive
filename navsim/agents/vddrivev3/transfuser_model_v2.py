...
from navsim.agents.vddrivev3.transfuser_backbone import TransfuserBackbone
from navsim.agents.vddrivev3.transfuser_features import BoundingBox2DIndex
from navsim.common.enums import StateSE2Index
from diffusers.schedulers import DDIMScheduler
from navsim.agents.vddrivev3.modules.conditional_unet1d import ConditionalUnet1D,SinusoidalPosEmb
import torch.nn.functional as F
from navsim.agents.vddrivev3.modules.blocks import linear_relu_ln,bias_init_with_prob, gen_sineembed_for_position, GridSampleCrossBEVAttention
# from navsim.agents.vddrivev3.modules.multimodal_loss import LossComputer
...
class TrajectoryHead(nn.Module):
    ...
    def norm_odo(self, odo_info_fut, vec=point_minmax): # odo_info_fut ([64, 20, 8, 2])
        """
    ...
    def denorm_odo(self, odo_info_fut, vec=point_minmax):
        # 提取每个点的x和y的min、max
        x_mins = vec[:, 0, 0].to(odo_info_fut.device)  # 形状 (8,)
        x_maxs = vec[:, 0, 1].to(odo_info_fut.device)
        y_mins = vec[:, 1, 0].to(odo_info_fut.device)
    ...
    def forward_train_speed_anchor(self, ego_query,agents_query,bev_feature,bev_spatial_shape, status_encoding, status_feature, targets=None,global_img=None) -> Dict[str, torch.Tensor]:
        ...
        plan_anchor = (gt_trajs[...,:2]-speed_anchor).unsqueeze(1).repeat(1, self.n_trajs, 1, 1).reshape(bs * self.n_trajs, 8, 2).to(device)
        plan_anchor = plan_anchor.unsqueeze(1) # torch.Size([1280, 1, 8, 2])
        odo_info_fut = self.norm_odo(plan_anchor, vec=anchor_minmax) # torch.Size([1280, 1, 8, 2])
        timesteps = torch.randint(
            0, 1000,
            (bs * self.n_trajs,), device=device
        ) 
        ...
        for idx, poses_reg in enumerate(poses_reg_list):
            ...
            trajectory_loss_idx = F.l1_loss(poses, # pred
                                            torch.stack([gt_trajs] * self.n_trajs, dim=1) # gt
                                            ) 
            trajectory_loss += trajectory_loss_idx
            trajectory_loss_dict[f"trajectory_loss_{idx}"] = trajectory_loss_idx

        return {"trajectory": poses, "trajectory_loss": trajectory_loss, "trajectory_loss_dict":trajectory_loss_dict}   
    ...
    def forward_test_speed_anchor(self, ego_query,agents_query,bev_feature,bev_spatial_shape,status_encoding, status_feature, global_img) -> Dict[str, torch.Tensor]:
        ...
        img = self.norm_odo(plan_anchor, vec=anchor_minmax)
        # noise = torch.randn(img.shape, device=device)
        # noisy_trajs = self.denorm_odo(img) 
        ego_fut_mode = img.shape[1]
        for step_idx in denoise_steps:
